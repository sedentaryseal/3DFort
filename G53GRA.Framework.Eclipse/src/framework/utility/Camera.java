...
public class Camera implements Input{
	 ...
	 * State of key press.
	 */
	private int wKey = 0, aKey = 0, sKey = 0, dKey = 0, zKey = 0, xKey = 0, oneKey = 0, twoKey = 0, threeKey = 0, fourKey = 0;
	/**
...
/////// Camera Functions ////////////////////////////////////////////
	...
	public void update(float dT) {
		...
		if (zKey == 1){
			eye = PVector.sub(eye, PVector.mult(up, speed));	// Move down
		}
		if (xKey == 1){
			eye = PVector.add(eye, PVector.mult(up, speed));	// Move up
		}
		// Number keys change the view
		if (oneKey == 1){
			MyScene.changeView(1);
			firstPersonView();
		}
		if (twoKey == 1){
			MyScene.changeView(2);
		}
		if (threeKey == 1){
			MyScene.changeView(1);
			alternateView();
...
////// Directional Axes Methods /////////////////////////////////////
	 ...
	 */
	public void reset(){
		eye.set(0.f,-200.f,550.f);									// Initially situate at origin CHANGED from 0 to 100
	...
	// View that looks towards the castle and cannon, seeing planes move closer
	public void alternateView(){
		eye.set(20.f,-150.f,-550.f);									// Initially situate at origin
		view.set(0.f,0.f,1.f);									// Set view direction forward
		forward.set(0.f,0.f,-1.f);								// Forward axis follows view direction
		right.set(1.f,0.f,0.f);									// Right axis initially right along world coordinates
		up.set(0.f,1.f,0.f);									// Up is up
		parent.translate(0.f,0.f,0.f);
	}

	// View from the top right side of the sky box
	public void viewFromAbove(){
		eye.set(550.f,-550.f,550.f);									// Initially situate at origin
		view.set(-1.f,0.5f,-1.f);									// Set view direction forward
		forward.set(0.f,0.f,-1.f);								// Forward axis follows view direction
		right.set(1.f,0.f,0.f);									// Right axis initially right along world coordinates
		up.set(0.f,1.f,0.f);									// Up is up
		parent.translate(0.f,0.f,0.f);
	...
	// View from the castle next to the cannon and soldier
	public void firstPersonView(){
		eye.set(10.f,-150.f,225.f);
		//eye.set(10.f,-150.f,140.f);									// Initially situate at origin CHANGED from 0 to 100
		//eye.z = 0.5f*parent.height/PApplet.tan(PI/6.f);			// Default camera radius (depth)
		//eye.z = 100.f;
		view.set(0.f,0.f,-1.f);									// Set view direction forward
		forward.set(0.f,0.f,-1.f);								// Forward axis follows view direction
